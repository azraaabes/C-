...
namespace CSharpEgitimKampi601.Services
{
    public class CustomerOperations
    {
        ...
        {
            var connection = new MongoDbConnection(); //MongoDb bağlantı isteğinde bulunduk
            var customerConnection = connection.GetCustomerCollection(); //Tabloma bağlandım

            var document = new BsonDocument //Parametreleri gönderdim
            {
            ...
            customerConnection.InsertOne(document); //Ekleme işlemi yaptım
        } 
        ...
        public List<Customer> GetAllCustomer()
        {
            ...
            var customerCollection = connection.GetCustomerCollection(); //koleksiyona,tabloya bağlantı hazırlamış oldum
        ...
        public void UpdateCustomer(Customer customer)
        {
            var connection = new MongoDbConnection();
            var customerCollection = connection.GetCustomerCollection();
            var filter = Builders<BsonDocument>.Filter.Eq("_id", ObjectId.Parse(customer.CustomerId)); //ilgili id yakalıyor
